...
# --- 3. UI & STYLING (Syntax Fixed) ---
st.set_page_config(page_title="English Guru Pro", page_icon="⚡", layout="wide")

# Double {{ }} are used here to avoid f-string SyntaxError
st.markdown(f"""
    <style>
    .stApp {{ 
    ...
    h1, h2, h3 {{ color: {st.session_state.theme}; text-shadow: 0 0 10px {st.session_state.theme}55; text-align: center; }}
    </style>
...
if page == "🏠 Dashboard":
    ...
    # Generating some dummy history data to show the chart
    chart_data = pd.DataFrame({"XP": [10, 40, 25, 60, st.session_state.xp]})
    ...
    if st.session_state.boss_hp <= 0:
        ...
        atk = st.radio("Choose your weapon (Answer):", q['o'], key="boss_atk")
        if st.button("💥 STRIKE"):
            if atk == q['a']:
